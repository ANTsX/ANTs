 ...
 *
 *=========================================================================*/
#ifndef __itkTimeSeriesSurroundSubtractionImageFilter_hxx
#define __itkTimeSeriesSurroundSubtractionImageFilter_hxx
...
TimeSeriesSurroundSubtractionImageFilter<TInputImage, TOutputImage>
...
::GenerateOutputInformation()
{
  ...
  if( phyData )
    ...
    itkExceptionMacro( << "itk::TimeSeriesSurroundSubtractionImageFilter::GenerateOutputInformation "
                       ...
                       ThreadIdType threadId)
{
  ...
  unsigned int nPairs = this->GetInput()->GetLargestPossibleRegion().GetSize()[InputImageDimension - 1] / 2.0 - 1;
  ...
  while( !outIt.IsAtEnd() )
    ...
    for( unsigned int i = 0; i < nPairs; i++ )
      {
      idx[InputImageDimension - 1] = i * 2 + offset;
      float value1 = this->GetInput()->GetPixel( idx );
      idx[InputImageDimension - 1] = i * 2 + offset + 1;
      float midvalue = this->GetInput()->GetPixel( idx );
      idx[InputImageDimension - 1] = i * 2 + offset + 2;
      float value2 = this->GetInput()->GetPixel( idx );

      meanDiff += ( 0.5 * value1 + 0.5 * value2 - midvalue );
      }
    meanDiff /= nPairs;
