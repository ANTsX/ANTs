...
Optional arguments:
     ...
     -e   use double precision ( default 0 )
...
# This keeps the template shape stable over multiple iterations of template building
    ...
    echo "   ${WARP} -d ${dim} --float $USEDOUBLE -i ${template} -o ${template} -t [${templatename}0GenericAffine.mat,1] -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -r ${template}"
    echo "--------------------------------------------------------------------------------------"

    ${WARP} -d ${dim} --float $USEDOUBLE -i ${template} -o ${template} -t [${templatename}0GenericAffine.mat,1] -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -r ${template}
}
  ...
  then
    ...
    for (( i = 0; i < ${#IMAGESETARRAY[@]}; i+=$NUMBEROFMODALITIES ))
      do
        ...
        basecall="${ANTS} -d ${DIM} --float $USEDOUBLE -u 1 -w [0.01,0.99] -z 1 -r [${TEMPLATES[0]},${IMAGESETARRAY[0]},1]"
        ...
        for (( j = 0; j < $NUMBEROFMODALITIES; j++ ))
          do
            ...
            exe2="$exe2 ${WARP} -d $DIM --float $USEDOUBLE -i ${IMAGESETARRAY[$k]} -o $RIGID -t ${outdir}/rigid${i}_0GenericAffine.mat -r ${TEMPLATES[$j]}\n"
            pexe2="$exe2 ${WARP} -d $DIM --float $USEDOUBLE -i ${IMAGESETARRAY[$k]} -o $RIGID -t ${outdir}/rigid${i}_0GenericAffine.mat -r ${TEMPLATES[$j]} >> ${outdir}/job_${count}_metriclog.txt\n"
          done
...
while [[ $i -lt ${ITERATIONLIMIT} ]];
  do
    ...
    for (( j = 0; j < ${#IMAGESETARRAY[@]}; j+=$NUMBEROFMODALITIES ))
      do
        basecall="${ANTS} -d ${DIM} --float $USEDOUBLE -u 1 -w [0.01,0.99] -z 1"
          ...
          do
            ...
            if [[ $N4CORRECT -eq 1 ]];
              then
                ...
                warpexe=" $warpexe ${WARP} -d ${DIM} --float $USEDOUBLE -i ${REPAIRED} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS}\n"
                warppexe=" $warppexe ${WARP} -d ${DIM} --float $USEDOUBLE -i ${REPAIRED} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS} >> ${outdir}/job_${count}_metriclog.txt\n"
              else
                ...
                warpexe=" $warpexe ${WARP} -d ${DIM} --float $USEDOUBLE -i ${IMAGESETARRAY[$l]} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS}\n"
                warppexe=" $warppexe ${WARP} -d ${DIM} --float $USEDOUBLE -i ${IMAGESETARRAY[$l]} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS} >> ${outdir}/job_${count}_metriclog.txt\n"
              fi
