...
int ImageMath(int argc, char *argv[])
{
  ...
  for(  vfIter2.GoToBegin(); !vfIter2.IsAtEnd(); ++vfIter2 )
    ...
    if( strcmp(operation.c_str(), "m") == 0 )
      ...
      {
      result = 1.0f / (1.0f + exp(-1.0f * ( pix1 - 0.25f) / pix2) );
      }
...
//    bool SaveImages = false;
  ...
  while( !o_iter.IsAtEnd() )
    ...
    if( outputmask->GetPixel(o_iter.GetIndex() ) == 1 )
      ...
      classMeans[label] = (n - 1.0f) / ( n ) *  classMeans[label] + 1.0f / n * pix;
      ...
      while( !GHood.IsAtEnd() )
        ...
        if( p == lab )
          ...
          for( unsigned int i = 0; i < GHood.Size(); i++ )
            ...
            if( val2 >= 0.5f && GHood.GetPixel(i) != lab )
              {
              objectedge++;
              totaledge++;
              }
            else if( val2 < 0.5f && GHood.GetPixel(i) != lab )
              {
              backgroundedge++;
              totaledge++;
              }
...
int LabelThickness(      int argc, char *argv[])
{
  ...
  for( iIt.GoToBegin(); !iIt.IsAtEnd(); ++iIt )
    ...
    if( label > 0 )
      ...
      if( surface[label] > 0 )
        {
        thicknessprior = volume[label] / surface[label] * 2.0f * volumeelement;
        }
