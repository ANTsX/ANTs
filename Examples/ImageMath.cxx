...
int FitSphere(int argc, char *argv[])
{
  ...
  if( fn2.length() > 3 )
    {
    ReadImage<ImageType>(wmimage, fn2.c_str() );
    }
...
*/
      ...
      while( okfit && dorad <= MaxRad )
        ...
        if( p > 0 && radimage->GetPixel(ind) <= 0  )
          ...
          if( goodct == possct )
            ...
            {
            okfit = false; radimage->SetPixel(ind, radimage->GetPixel(ind) * (-1.0) );
            }
          }
        dorad = dorad + 1;
        }
  ...
  for( unsigned int j = 0; j < ImageDimension; j++ )
    {
    rad[j] = (long unsigned int)globalbestrad;
    }
  iteratorType GHood(rad, image1, image1->GetLargestPossibleRegion() );
...
// square image
  ...
  for( unsigned int roi = 1; roi <= maxlab; roi++ )
    ...
    if( wmroimap.find(roi) != wmroimap.end() && iswm )
      {
      std::cout << wmroimap.find(roi)->second << " & " << clusters[roi] << " , "  << pvals[roi] << "  & xy & yz  \\ "
                << std::endl;
      }
...
int SegmentImage(      int argc, char *argv[])
{
  ...
  if( argc > argct )
    {
    kclasses = atoi(argv[argct]);
    }
  argct++;
  float locweight = 0.5;
  if( argc > argct )
    {
    locweight = atof(argv[argct]);
    }
  argct++;
  if( locweight < 0 )
    {
  ...
  if( argc > argct )
    {
    priorfn1 = std::string(argv[argct]);
    }
  argct++;
  std::string priorfn2 = "";
  if( argc > argct )
    {
    priorfn2 = std::string(argv[argct]);
    }
  argct++;
  std::string priorfn3 = "";
  if( argc > argct )
    {
    priorfn3 = std::string(argv[argct]);
    }
  argct++;
  if( kclasses < 1 )
    {
        ...
        if( image->GetPixel(vfIter2.GetIndex() ) > 0 )
          ...
          if( total > 0 )
            ...
            if( px1 > px2 && px1 > px3 )
              ...
              if( diff < 0 )
                {
                px1 -= diff;  px2 += diff * 0.5;  px3 += diff * 0.5;
                }
              }
              ...
              if( diff < 0 )
                {
                px2 -= diff;  px1 += diff * 0.5;  px3 += diff * 0.5;
                }
              }
              ...
              if( diff < 0 )
                {
                px3 -= diff;  px2 += diff * 0.5;  px1 += diff * 0.5;
                }
              }
  ...
  for( unsigned int iters = 0; iters < 2; iters++ )
    ...
    for( unsigned int k = 1; k < kclasses; k++ )
      ...
      if( ct == 0 )
        {
        std::cout << " bad initialization " << std::endl; ct = 1;
        }
      if( lct == 0 )
        {
        std::cout << " bad template initialization " << std::endl; lct = 1;
        }
      ...
      for(  vfIter2.GoToBegin(); !vfIter2.IsAtEnd(); ++vfIter2 )
        ...
        if( k == 1 && p1image )
          {
          estimatedmean = p1image->GetPixel(vfIter2.GetIndex() ) * locweight + gestimatedmean * gwt;
          }
        if( k == 2 && p2image )
          {
          estimatedmean = p2image->GetPixel(vfIter2.GetIndex() ) * locweight + gestimatedmean * gwt;
          }
        if( k == 3 && p3image )
          {
          estimatedmean = p3image->GetPixel(vfIter2.GetIndex() ) * locweight + gestimatedmean * gwt;
          }
