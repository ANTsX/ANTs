...
int
antsJointFusion(itk::ants::CommandLineParser * parser)
{
  ...
  {
    std::cout << std::endl
              << "Running antsJointFusion for " << ImageDimension << "-dimensional images." << std::endl
              << std::endl;
  }
...
void
ajfInitializeCommandLineOptions(itk::ants::CommandLineParser * parser)
{
  ...
  {
    std::string description = std::string("Search radius for similarity measures.  Default = 3x3x3.  One ") +
...
// 'main()'
int
antsJointFusion(std::vector<std::string> args, std::ostream * /*out_stream = nullptr */)
{
