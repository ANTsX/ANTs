                  ...
                  unsigned int svd_option = 0, PixelType usel1 = 1 )
{
  ...
  const bool have_p_mask = ReadImage<ImageType>(mask1, option->GetFunction( 0 )->GetParameter( 1 ).c_str() );
...
template <unsigned int ImageDimension, class PixelType>
int SCCA_vnl( itk::ants::CommandLineParser *parser, unsigned int permct, unsigned int newimp,
...
int sccan( itk::ants::CommandLineParser *parser )
{
  ...
  if( matrixPairOption && matrixPairOption->GetNumberOfFunctions() > 0 )
    ...
    if(  !initializationStrategy.compare( std::string( "two-view" ) )  )
      ...
      exitvalue = SCCA_vnl<ImageDimension, double>( parser, permct, eigen_imp, robustify, p_cluster_thresh,
                                                    q_cluster_thresh,
                                                    iterct, usel1 );
      }
