...
::ComputeUpdateField(DeformationFieldPointer fixedwarp, DeformationFieldPointer movingwarp,   PointSetPointer fpoints,
                     PointSetPointer wpoints, DeformationFieldPointer totalUpdateInvField,
                     bool updateenergy)
{
...
//	std::cout << " C " << std::endl;
    ...
    if( movingwarp )
      {
      wfimage =
        this->WarpMultiTransform( this->m_SmoothFixedImages[metricCount], this->m_SmoothFixedImages[metricCount], NULL,
                                  movingwarp, false,
                                  this->m_FixedImageAffineTransform );
      }
    else
...
::ComputeUpdateFieldAlternatingMin(DeformationFieldPointer fixedwarp, DeformationFieldPointer movingwarp,
                                   ...
                                   DeformationFieldPointer totalUpdateInvField,
                                   bool updateenergy)
{
  ImagePointer mask = NULL;
...
//	if (i == j) iddir[i][j]=1;
  ...
  if( generatetvfield )
    ...
    this->m_TimeVaryingVelocity->SetSpacing(gspace);
    this->m_TimeVaryingVelocity->SetOrigin(gorigin);
    //        if (this->m_Debug)  std::cout << " Field size " <<
    // this->m_TimeVaryingVelocity->GetLargestPossibleRegion().GetSize() << std::endl;
    }
  if( !this->m_SyNF )
    {
    std::cout << " Allocating " << std::endl;
...
/** NOte, totalUpdateInvField is filled with zeroes! -- we only want
    ...
    //      std::cout << " t " << hit <<  " F " << this->MeasureDeformation(this->m_SyNFInv ) << " F1 " <<
    // this->MeasureDeformation( this->m_SyNMInv ) << std::endl;
    }
